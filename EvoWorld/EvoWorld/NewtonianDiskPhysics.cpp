...
Vector2<double> NewtonianDiskPhysics::calculate_friction(const PhysicsDisk& disk, const std::chrono::duration<double>& time_step) const
{
    ...
    auto friction_magnitude = disk.get_mass() * coefficient_of_friction_;
...
std::unordered_map<std::shared_ptr<PhysicsDisk>, std::shared_ptr<PhysicsDisk>> NewtonianDiskPhysics::detect_collisions(
    const std::vector<std::shared_ptr<PhysicsDisk>>& disks)
{
    std::unordered_map<std::shared_ptr<PhysicsDisk>, std::shared_ptr<PhysicsDisk>> collisions;
...
void NewtonianDiskPhysics::resolve_collision(PhysicsDisk& disk, PhysicsDisk& other_disk)
{
...
void NewtonianDiskPhysics::update(const std::chrono::high_resolution_clock::time_point& now, const std::chrono::duration<double>& time_step,
                   ...
                   const std::unordered_map<std::shared_ptr<PhysicsDisk>, Vector2<double>>& forces)
{
    ...
    {
        const auto friction = calculate_friction(*disk, time_step);
